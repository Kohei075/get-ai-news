...
4. 既存のニュースファイルとの重複を排除する:
   - `news/YYYY-MM/` ディレクトリ内のファイルをGlobで確認（月別フォルダ構成）
   - **過去7日分のニュースファイルをすべて読み込み、既出のニュースタイトル・トピックをリストアップする**（月をまたぐ場合は前月のフォルダも確認する）
   - 今日の日付のファイルが既に存在する場合も内容を読む
   - **過去に掲載済みのニュース（同じ発表・同じイベント・同じ資金調達など、主題が同一のもの）は選ばない**
   - 検索結果に過去と同じニュースしか出てこない場合は、追加の検索クエリ（別の切り口・別のトピック）で新しいニュースを探す

5. 取得結果を整理し、`news/YYYY-MM/YYYY-MM-DD-ai-news.md` として保存する（月別フォルダがなければ作成する）。フォーマット:
